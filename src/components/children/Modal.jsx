...
const Modal = ({ closeModal, dispatch, id }) => {
      ...
      <div className="fixed z-50 bg-[#00000050] w-screen p-4 overflow-x-hidden overflow-y-auto top-0 left-0 h-screen md:min-h-screen">
         <div className="relative w-full h-full max-w-md md:h-auto top-1/2 left-1/2 -translate-x-2/4 -translate-y-2/4">
            <div className="relative bg-white rounded-lg shadow">
               ...
               <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-6 lg:px-8">
                  <div className="space-y-6">
                     <h1 className="mb-4 text-xl font-medium text-gray-900 dark:text-white">Adit</h1>
                     <div>
                        <input {...register("title", { required: true })} placeholder="adit task" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required />
                     </div>
                     <button className="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">adit</button>
